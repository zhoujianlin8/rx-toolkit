...
module.exports = {
  ...
  configPathExists: function () {
    if(!fs.existsSync(RT.configPath)){
      console.log('当前目录rt-config.js 不存在，请切换正确目录')
      process.exit(1)
    }
  },
  ...
  getWebpackConfig: function(options){
    return new Promise(function (reslove) {
      ...
      if(util.isFunction(RT.getWebpackConfig)){
        var config = RT.getWebpackConfig(RT) || webpackConfig;
        if(util.isFunction(config.then)){
          config.then(function (data) {
            reslove(data || webpackConfig)
          })
        }else{
          reslove(config);
        }
      }else{
        reslove(webpackConfig);
      }
    });
  },
